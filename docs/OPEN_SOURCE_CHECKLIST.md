...
- [ ] Validate the local checkpoint with `python -m rdcl.tools.validate_checkpoint`.
...
Recommended local-only checkpoint filename:

```text
checkpoints/rdcl_30m_state_dict.pt
```

This file is ignored by `.gitignore` and should not be pushed by default.
